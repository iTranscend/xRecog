...
class XrecogMainWindow(QtWidgets.QMainWindow, EventEmitter):
    ...
    def buildReport(self):
        print("<buildReport> Building Report")
        ...
        print("<buildReport> Report Collated")
        print("<buildReport> Merging markdown lines")
        markdown = "\n".join(markdown)
        print("<buildReport> Merged markdown lines")
        print("<buildReport> Creating Document View")
        document = QtGui.QTextDocument()
        document.setMarkdown(markdown)
        print("<buildReport> Document view created")
        return document
    ...
    def showReportPreview(self, type):
        ...
        print("<showReportPreview> Showing report preview in %s format" % type)

    def printHTML(self):
        print("<printHTML> Printing HTML")
        filename = 'report.html'
        document = self.buildReport()
        print("<printHTML> Saving requested HTML report to %s" % filename)
        with open(filename, 'w') as file:
            file.write(document.toHtml())
        print("<printHTML> Successfully saved HTML report")

    def printMarkdown(self):
        print("<printMarkdown> Printing Markdown")
        filename = 'report.md'
        document = self.buildReport()
        print("<printMarkdown> Saving requested Markdown report to %s" % filename)
        with open(filename, 'w') as file:
            file.write(document.toMarkdown())
        print("<printMarkdown> Successfully saved Markdown report")

    def print(self):
        print("<print> Printing document")
        document = self.buildReport()
        print("<print> Starting Print")
        dialog = QtPrintSupport.QPrintDialog()
        if dialog.exec_() == QtWidgets.QDialog.Accepted:
            document.print_(dialog.printer())
        print("<print> Successfully printed")

    def printPreview(self):
        print("<printPreview> Printing Preview")
        document = self.buildReport()
        print("<printPreview> Start rendering")
        dialog = QtPrintSupport.QPrintPreviewDialog()
        dialog.paintRequested.connect(lambda printer: document.print_(printer))
        dialog.exec_()
        print("<printPreview> Successfully printed")
